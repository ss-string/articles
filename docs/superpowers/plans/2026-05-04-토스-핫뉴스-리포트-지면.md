# 토스 핫뉴스 리포트 지면 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Supabase `toss_wts_hot_news_reports` 데이터를 카드 지면으로 보여주고, 카드 선택 시 row 기반 리포트 팝업을 띄운다.

**Architecture:** 기존 컨센서스 기능처럼 데이터 조회, 정규화, 상태 훅, 화면 컴포넌트를 분리한다. `report_payload`를 우선 사용하고 top-level 컬럼을 fallback으로 사용해 `HotNewsReport` 표시 모델을 만든다. 팝업은 독립 컴포넌트로 두고 딤드, 닫기 인터랙션, 링크 처리를 담당한다.
...
- Create: `src/hot-news/model.ts`
  - `RawHotNewsReportRow`, `HotNewsReport`, `HotNewsArticle`, `HotNewsCompanyEvidence` 타입과 정규화 함수 담당.
- Create: `src/hot-news/model.test.ts`
  - 실제 Supabase row 구조 샘플, fallback, 예외, 날짜 포맷 테스트.
- Create: `src/hot-news/api.ts`
  - `toss_wts_hot_news_reports` 조회 함수 담당.
- Create: `src/hot-news/useHotNewsReports.ts`
  - loading/success/error 상태 관리.
- Create: `src/hot-news/useHotNewsReports.test.tsx`
  - 훅의 성공, 실패, 환경변수 누락 테스트.
- Create: `src/components/HotNewsReportModal.tsx`
  - 리포트 팝업, 딤드, 닫기 인터랙션, 주요 기사 링크 표시 담당.
- Create: `src/components/HotNewsReportsPage.tsx`
  - 카드 지면과 선택 상태 관리.
- Create: `src/components/HotNewsReportsPage.test.tsx`
...
Expected: FAIL. `src/hot-news/model` 모듈이 없다는 오류가 나야 한다.
...
Expected: PASS. `5`개 테스트가 모두 통과해야 한다.
...
Expected: FAIL. `useHotNewsReports` 모듈이 없다는 오류가 나야 한다.
...
Expected: PASS. `3`개 테스트가 모두 통과해야 한다.
...
Expected: FAIL. `HotNewsReportsPage` 모듈이 없다는 오류가 나야 한다.
...
Modify `src/App.test.tsx` so the first test renders hot news too:

```tsx
import type { RawHotNewsReportRow } from './hot-news/model';
```

Add below the existing `rows` fixture:
...
Expected: FAIL. `queryHotNewsRows` prop이 `App`에 없거나 핫뉴스 내비게이션이 없다는 실패가 나야 한다.

- [ ] **Step 3: 내비게이션 수정**

Modify `src/content.ts` navItems:
...
Render the new page before the consensus page:
...
Append to `src/styles.css` before the first media query:
...
Expected: PASS. 앱 테스트와 핫뉴스 컴포넌트 테스트가 모두 통과해야 한다.
...
Expected: 모든 테스트가 PASS. 기존 Vitest `--localstorage-file` 경고는 기준 상태에서도 있었으므로 새 실패로 보지 않는다.
...
Expected: TypeScript build와 Vite build가 모두 성공해야 한다.
...
Expected: 작업 중 생성한 커밋 외에 uncommitted 변경이 없어야 한다.

- [ ] **Step 4: 검증 커밋이 필요한 경우만 커밋**
...
Expected: 수정이 없으면 이 단계는 실행하지 않는다.

---

## 자체 검토

- 설계 범위의 카드 지면, 팝업, 딤드, X 버튼, 주요 기사 링크, 데이터 fallback, 빈 상태, 오류 상태가 각 Task에 포함되어 있다.
- 컨센서스 랭킹 지면의 기존 계산과 UI를 변경하지 않는다.
- 모든 구현 Task는 실패 테스트 확인 후 최소 구현과 통과 확인을 거친다.
