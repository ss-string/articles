...
**Goal:** `금융 > AI 분석 리포트` 최초 진입 시 검색값과 선택 종목을 비우고, 검색어가 없을 때 종목별 최신 리포트 기준 `total_score` 상위 10개 종목을 추천 칩으로 보여준다.
...
- Modify: `src/components/AiAnalysisReportsPage.tsx`
  - 검색어가 없을 때 사용할 종목별 최신 리포트 기준 추천 목록 파생값을 만들고, 자동 선택 fallback을 제거한다.
...
- 설계서의 최초 검색 빈 값, 자동 선택 제거, 종목별 최신 리포트 기준 `total_score` 상위 10개 노출 요구를 Task 1과 Task 2가 모두 다룬다.
